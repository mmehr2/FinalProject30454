...
if __name__ == "__main__":
    ...
    parsedValuesH = dataValuesH.map(lambda x: re.sub(r"\"humidity\":", "", x)).map(lambda x: re.sub(r",", "", x)).map(float)
    parsedValuesL = dataValuesL.map(lambda x: re.sub(r"\"ambient_light\":", "", x)).map(lambda x: re.sub(r",", "", x)).map(float)
    ...
    sumMapH = parsedValuesH.reduce(add)
    sumMapL = parsedValuesL.reduce(add)
    valueSumH = sumMapH.map(lambda x: "Total Sum of Humidity: " + unicode(x))
    valueSumL = sumMapL.map(lambda x: "Total Sum of Light: " + unicode(x))
    valueSumH.pprint()
    valueSumL.pprint()
